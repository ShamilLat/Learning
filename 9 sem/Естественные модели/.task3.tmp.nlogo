...
to setup
  ...
  [
    set shape "circle"
    set size 0.5
    set my-x ((random-float dist-x) + min-x?)
...
to move-turtle
  let new-x my-x + vel * (dist-x - random-float (dist-x * 2))
  if new-x < min-x? or new-x > max-x? [stop]
  let old-y func my-x
...
to draw-target
  create-turtles 1 [
    set color black
    let i 0
    setxy get-xcor min-x? get-ycor func min-x?
    pendown
    let rep-cnt (dist-x * 200)
    repeat rep-cnt [
      set i i + 1
...
to-report get-xcor [x]
  report min-pxcor + (x - min-x?) / dist-x * (max-pxcor - min-pxcor)
end
...
"average" 1.0 0 -11085214 true "" "plot average-target"
...
PENS
"dist" 1.0 1 -14070903 true "set-histogram-num-bars 50" "histogram [my-x] of turtles"
