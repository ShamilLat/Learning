...
int main(int argc, char** argv) {
  ...
  if (rank == 0)
    b_local = (double*)calloc(size, sizeof(double));
  ...
  for (uint32_t i = 0; i < size / world_size; i++)
    MPI_Scatter(A.val + i * world_size * size, size, MPI_DOUBLE, columns[i],
                size, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  for (uint32_t i = 1; i < disps; i++) {
    if (rank == 0)
      MPI_Send(A.val + size * world_size * (size / world_size) + i * size, size,
               MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
    else if (rank == i)
      MPI_Recv(columns[columns_number - 1], size, MPI_DOUBLE, 0, 0,
  ...
  double* a_k = (double*)malloc(size * sizeof(double));
  ...
  for (uint32_t i = 0; i < size - 1; i++) {
    ...
    if (rank == i % world_size) {
      for (uint32_t j = i; j < nrows; j++)
        a_k[j - i] = columns[i / world_size][j];

      double a_k_norm = get_norm(a_k, len);
      a_k[0] += sgn(a_k[0]) * a_k_norm;
      a_k_norm = get_norm(a_k, len);

      for (uint32_t j = 0; j < len; j++)
        a_k[j] /= a_k_norm;
    }

    MPI_Bcast(a_k, len, MPI_DOUBLE, i % world_size, MPI_COMM_WORLD);
    ...
    for (uint32_t col = i / world_size; col < columns_number; col++) {
      if (col * world_size + rank >= i) {
        ...
        for (uint32_t k = 0; k < len; k++)
          sum += 2.0 * a_k[k] * columns[col][k + i];
        for (uint32_t k = 0; k < len; k++)
          columns[col][k + i] -= sum * a_k[k];
      }
    }
  ...
  if (size % world_size != 0) {
    if (rank == size % world_size) {
      ...
      MPI_Recv(b_local, size, MPI_DOUBLE, size % world_size, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    }
  } else if (rank == 0) {
  ...
  for (int32_t i = size - 1; i >= 0; i--) {
    if (rank == i % world_size) {
      if (world_size > 1)
        MPI_Recv(b, size, MPI_DOUBLE, (i + 1) % world_size, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
      ...
      if (world_size > 1)
        MPI_Send(b, size, MPI_DOUBLE, i % world_size, 0, MPI_COMM_WORLD);
    }
  }
  ...
  MPI_Allreduce(&local_hh_time, &hh_time, 1, MPI_DOUBLE, MPI_MAX,
                MPI_COMM_WORLD);
  MPI_Allreduce(&local_gs_time, &gs_time, 1, MPI_DOUBLE, MPI_MAX,
                MPI_COMM_WORLD);
  MPI_Allreduce(&local_all_time, &all_time, 1, MPI_DOUBLE, MPI_MAX,
                MPI_COMM_WORLD);

  for (uint32_t i = 0; i < size / world_size; i++)
    MPI_Gather(columns[i], size, MPI_DOUBLE, A.val + i * world_size * size,
               size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  for (uint32_t i = 1; i < disps; i++) {
    if (rank == i)
      MPI_Send(columns[columns_number - 1], size, MPI_DOUBLE, 0, 0,
               MPI_COMM_WORLD);
    else if (rank == 0)
      MPI_Recv(A.val + world_size * size * (size / world_size) + i * size, size,
...
/*   if (rank == 0) {
  ...
  if (rank == 0) {
    ...
    std::cout << "||Ax-b|| = " << get_discrepancy(A, x_global, b_local)
              << std::endl;
