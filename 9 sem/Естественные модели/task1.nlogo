...
to setup-turtles
  create-turtles turtles-number? [setxy random-xcor random-ycor]
end
...
to regrow-grass
  ask patches [
    if random 100 < (grass-probabilty? + 1) [set pcolor green]
  ]
end
...
to move-turtles
  ask turtles [
    ifelse turtles-speed? [set speed 0.5] [set speed 0.1]
    right max-angle? - random (max-angle? * 2)
    forward speed
